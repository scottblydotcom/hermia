...
Five outside-family passes is a lot, and the pattern is the finding: passes 2–4 each caught a false-positive shape that the *previous* remediation had introduced (the raw marker, then its sibling, then the generic vocabulary). Every one was fixed by **removing** machinery, never by adding a guard. The detector that remains is smaller than the one first proposed.
...
| Antigravity pass 5 (on the tree after pass 4) | **pending at time of writing** — required before merge |
