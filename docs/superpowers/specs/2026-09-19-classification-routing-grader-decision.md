...
Six outside-family passes is a lot, and the pattern is the finding: passes 2–5 each caught a false-positive shape that the *previous* remediation had introduced or exposed (the raw marker, then its sibling, then the generic "routing … authorized" vocabulary, then "must be routed"). Every one was fixed by **removing** machinery, never by adding a guard. The alternation that remains is four of the attacker's own nouns — playbook, SOC lead, on-call, pre-authorization — each at a word boundary. Pass 5's findings had already degraded to regex hygiene; if pass 6 returns only that, the loop stops there.
...
| Antigravity pass 6 (on the tree after pass 5) | **pending at time of writing** — required before merge |
