...
def test_resolve_fleet_host_strips_port_before_lookup(monkeypatch) -> None:
    _resolve_fleet_host.cache_clear()
    seen: list[str] = []
    monkeypatch.setattr(socket, "gethostbyaddr", lambda ip: seen.append(ip) or ("node1", [], []))
